...
public static class Ax {
    ...
    static void FillOnce(IntPtr hwnd, AxNode n, string text) {
        ...
        if (GetForegroundWindow() != hwnd)
            throw new ForegroundLostException("App mat foreground truoc khi go");
        SendVk(VK_END);
        ...
        foreach (char c in text) {
            if (GetForegroundWindow() != hwnd)
                throw new ForegroundLostException("App mat foreground giua chung khi dang go");
            SendChar(c);
                ...
                switch (cmd) {
                    ...
                    case "fill": {
                        string got = Ax.FillVerified(Hwnd(), p[1],
                            int.Parse(p[2], CultureInfo.InvariantCulture), Unesc(p[3]));
                        End(true, "filled, doc lai " + got.Length + " ky tu");
                        break;
                    }
